...
const AccordionMultiselectContext =
   React.createContext<AccordionMultiselectContextValue | null>(null)

function useAccordionMultiselect() {
   const context = React.useContext(AccordionMultiselectContext)
   if (!context) {
      throw new Error(
         'AccordionMultiselect subcomponents must be used within AccordionMultiselect'
      )
   }
   return context
}
...
interface AccordionMultiselectProps extends Omit<
   AccordionRootProps,
...
function AccordionMultiselect({
   ...
   return (
      <AccordionMultiselectContext.Provider
         value={{ selectedValues, onSelectionChange: handleSelectionChange }}
      >
...
function AccordionMultiselectContent({
   ...
   return (
      <AccordionPrimitive.Content
         ...
         {...props}
      >
         <div className={cn('pt-2 pb-4 flex flex-col gap-2', className)}>
            {children}
         </div>
      </AccordionPrimitive.Content>
   )
            ...
            <div className='flex items-center h-5 mt-1'>
               <CheckboxPrimitive.Root
                  ...
                  onCheckedChange={checked =>
                     onSelectionChange(value, checked as boolean)
                  }
