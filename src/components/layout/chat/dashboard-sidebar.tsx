...
import { authClient } from "@/lib/auth-client"; // Assuming this exists
...
// 1. Define the shape of a sidebar item
...
// 2. Centralized Data for Sidebar
...
export default function DashboardSidebar() {
  ...
  // Mocking session for the example as per your original code
  // In production: const { data: session } = authClient.useSession()
  const session = {
    user: {
      name: "Amanuel Antenh",
        ...
        <SidebarGroup className="px-2 py-4">
          <nav className="space-y-1">
            {sidebarItems.map((item) => {
              // 3. Determine if this link is active
