// src/components/layout/settings/account-settings.tsx
"use client";
...
export default function AccountSettings() {
  ...
  const typedSession = session as unknown as UserSession;

  const handleAvatarUpdated = (newImageUrl: string) => {
    if (typedSession.user) {
      typedSession.user.image = newImageUrl;
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground text-xs text-center">
        Update your personal details, email, bio, password, and profile picture
        also add job and location.
      </p>
      ...
      <div className="mt-5 p-2 flex flex-col gap-2 md:grid md:grid-cols-2 justify-center">
        <div className="w-full flex justify-center items-center col-span-2">
          <ProfileDropdown key={avatarKey} data={typedSession.user} />
        </div>
        ...
        {isMobile ? (
          <>
            ...
            <Drawer>
              ...
              <DrawerContent>
                <DrawerTitle className="sr-only">Personal Details</DrawerTitle>
            ...
            <Drawer>
              ...
              <DrawerContent>
                ...
                <EmailOperation session={typedSession} />
              </DrawerContent>
            </Drawer>
          ...
          <div className="md:grid md:grid-cols-2 lg:grid-cols-3">
            <p className="text-sm text-muted-foreground my-2">
              ...
              <span className="text-foreground font-black">
                {new Date(typedSession.user.createdAt).toLocaleDateString()}
              </span>
            </p>
            <p className="text-sm text-muted-foreground my-2">
              Last profile update at{" "}
              <span className="text-foreground font-black">
                {new Date(typedSession.user.updatedAt).toLocaleDateString()}
              </span>
            </p>
            <p className="text-sm text-muted-foreground my-2">
              <span className="text-foreground font-black">52</span> Total Chat
              Contacts
            </p>
            <p className="text-sm text-muted-foreground my-2">
              <span className="text-foreground font-black">342</span> Total
              Messages
            </p>
            <p className="text-sm text-muted-foreground my-2">
              <span className="text-foreground font-black">2</span> Total Pinned
              Chats
            </p>
            <p className="text-sm text-muted-foreground my-2">
